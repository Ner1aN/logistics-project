...
Ключевая особенность модели — связь `TransportationRequest` и `Transportation` в формате 1:N. Это позволяет моделировать реальные ситуации, когда одна заявка выполняется несколькими ТС или несколькими рейсами.
